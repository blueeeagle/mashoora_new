...
(function ($) {
    ...
    $.fn.inputVal = function (newValue) {
        var $self = $(this);
        if($.isEmptyObject(newValue)) return $self
        ...
        if($self.is('input, textarea, select')) {
            ...
            if(typeof newValue === 'undefined') {
                ...
                foreach(newValue, function (value, inputName) {
                    ...
                    $self.find(`[name="${inputName}"]`).val(value)

                    if($self.find(`[name="${inputName}"]`).hasClass('checkboxOnOff')){
                        if(value.split(",").length >= 2) {
                            $self.find(`[name="${inputName}"]`).val(value.split(",")[0])
                            $self.find(`[name="${inputName}"]`).prop( "checked",false)
                        }else{
                            $self.find(`[name="${inputName}"]`).prop( "checked",true)
                        }
                    }
                });
                return $self;
